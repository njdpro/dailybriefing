...
// The temporary directory is deleted after FFmpeg finishes,
// so the final MP3 must live outside it.
...
if (!data.audioContent) {
  throw new Error(`Google TTS returned no audio: ${body.slice(0, 500)}`);
}

fs.writeFileSync(
  path.join(tempDir, `part-${String(i).padStart(4, "0")}.mp3`),
...
(_, i) => `part-${String(i).padStart(4, "0")}.mp3`
);
...
try {
// FIX: Write the finished MP3 to finalOutput, which is
// outside tempDir and therefore survives cleanup.
execFileSync("ffmpeg", [
...
```
// FIX: Verify FFmpeg actually created the expected file.
...
console.log(
  `TTS complete: ${finalOutput} (${finalStats.size} bytes)`
);
```
...
// Safe because finalOutput is outside tempDir.
fs.rmSync(tempDir, { recursive: true, force: true });
}
}
...
const itemXml = `   <item>     <title>Davis Briefing — ${escapeXml(date.toFormat("MMMM d, yyyy"))}</title>     <description>Daily morning news briefing for the Davis family.</description>     <enclosure url="${escapeXml(`${BASE_URL}/${filename}`)}" length="${fileSize}" type="audio/mpeg"/>     <pubDate>${date.toUTC().toString()}</pubDate>     <guid isPermaLink="false">davis-briefing-${date.toFormat("yyyy-MM-dd")}</guid>   </item>`;

const items = [...existing.matchAll(/<item>[\s\S]*?</item>/gi)].map(m => m[0]);
const newItems = [
itemXml.trim(),
...items.filter(
item => !item.includes(`davis-briefing-${date.toFormat("yyyy-MM-dd")}`)
)
...
throw new Error("rss.xml has no channel element");
}
...
console.log(
`Script: ${script.length} characters, ~${script.split(/\s+/).length} words`
);
